...
def build_config(args):
    ...
    if not input_root:
        osppath.list_path(show=True, current=None)
    ...
    if arg.quick:
        board = osppath.support_board(osp_root)[0]
        config["BOARD"] = board
        bd_ver = osppath.get_board_version(osp_root, board)[0]
        config["BD_VER"] = bd_ver
        cur_core = osppath.get_tcfs(osp_root, board, bd_ver)[0]
        config["CUR_CORE"] = cur_core
        config["TOOLCHAIN"] = SUPPORT_TOOLCHAIN
        return config
...
def setup(subparser):
    ...
    subparser.add_argument(
        '--quick', action='store', default="", help='Create application quickly')
